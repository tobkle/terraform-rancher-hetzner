resource "null_resource" "rancher_configuration" {
  ...
  provisioner "local-exec" {
    command = "scp -i \"~/.ssh/${var.ssh_key_name}\" -o \"StrictHostKeyChecking=no\" -r \"${var.user}@${format(var.hostname_format, 1)}.${var.domain}:/tmp/rancher_node.sh\" \"${path.module}/rancher_node.sh\"",
  }
}
